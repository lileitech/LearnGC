...
public:
	...
	typedef arc* arc_id;
	arc_id get_first_arc();//i->j
	arc_id get_next_arc(arc_id a);//j->i
...
private:
	...
	struct arc//------------边  （假设是i->j）
	{
		node		*head;		// node the arc points to(指的是j吗？)
		arc			*next;		// next arc with the same originating node (相当于是i->k?)
		arc			*sister;	// reverse arc (相当于是j->i?)
	...
	//----为什么是这样的呢？nodes，arcs指的是什么呢？
	int					node_num;

	DBlock<nodeptr>		*nodeptr_block;//------这个又是什么？
	...
	Block<node_id>		*changed_list;//-----------这个使用干什么的呢？

	/////////////////////////////////////////////////////////////////////////

	node				*queue_first[2], *queue_last[2];	// list of active nodes 什么叫做list？
...
inline void Graph<captype, tcaptype, flowtype>::add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
	...
	tcaptype delta = nodes[i].tr_cap;//判断正负，确定是source还是sink
	if (delta > 0) cap_source += delta;
	else           cap_sink -= delta;
	flow += (cap_source < cap_sink) ? cap_source : cap_sink;//-----为什么flow反而取小的cap？？
...
inline void Graph<captype, tcaptype, flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
	...
	arc *a = arc_last++;//----这里的a就是一种arc；其中的arc_last = arcs+2*edge_num
	arc *a_rev = arc_last++;

	node* i = nodes + _i; //-----nodes是什么？
	node* j = nodes + _j;

	a->sister = a_rev;//------为什么所有的赋值感觉都是反过来的？
	a_rev->sister = a;
	a->next = i->first;
	i->first = a;
	a_rev->next = j->first;
	j->first = a_rev;
	a->head = j;
	a_rev->head = i;//在此之前都是在构造a和a_rev，最后才是cap赋值
	a->r_cap = cap;
	a_rev->r_cap = rev_cap;
}
