...
(and the second queue becomes empty).****-----*****？？两个queue，所以才对应有*queue_first[2], *queue_last[2]吗？
...
inline typename Graph<captype, tcaptype, flowtype>::node* Graph<captype, tcaptype, flowtype>::next_active()
{
	...
	while (1) //if it is connected to the sink, it stays in the list 
	{
		if (!(i = queue_first[0]))
		{
...
/***********************************************************************/
//max flow的初始化---弄清楚tr_cap 怎么求？--在add_tweights确定
template <typename captype, typename tcaptype, typename flowtype>
void Graph<captype, tcaptype, flowtype>::maxflow_init()
{
	...
	for (i = nodes; i<node_last; i++)
	{
		...
		i->TS = TIME;
		if (i->tr_cap > 0)//直接根据capaticy来定义正向还是反向？
		{
...
// “增广”阶段--找出最小capacity，确定orphan--弄清楚r_cap 怎么求？---在add_edge确定
//---确定r_cap和tr_cap这两个cap之间的比重关系

template <typename captype, typename tcaptype, typename flowtype>
void Graph<captype, tcaptype, flowtype>::augment(arc *middle_arc)
{
	node *i; //i由middle_arc来确定，也就是 i = middle_arc->sister->head
	arc *a; //a再由i确定，也就是 a = i->parent;
	tcaptype bottleneck;//最小capacity，也即是最大流，（注意是tcaptype）


	/* 1. Finding bottleneck capacity */
	/* 1a - the source tree */
	bottleneck = middle_arc->r_cap;// residual capacity
	for (i = middle_arc->sister->head;; i = a->head)//从头到尾
	{
		a = i->parent;
		if (a == TERMINAL) break;
		if (bottleneck > a->sister->r_cap) bottleneck = a->sister->r_cap;//先和r_cap比较，取小的
	}
	if (bottleneck > i->tr_cap) bottleneck = i->tr_cap;//再和tr_cap比较，取小的
	/* 1b - the sink tree */
	...
	if (bottleneck > -i->tr_cap) bottleneck = -i->tr_cap;//目前还没有找到他们之间权重关系的“reda”系数
	...
	for (i = middle_arc->sister->head;; i = a->head)
	{
		...
		if (a == TERMINAL) break;
		a->r_cap += bottleneck;
		a->sister->r_cap -= bottleneck;//a的顺序就是合乎逻辑的，why?
		if (!a->sister->r_cap)
		{
