...
int main(int argc, char* argv[])
{
		...
		//mainfold = "E:\\LA_Segmentation\\test";
		//string  casename = "P01036_6";
		...
		// first line number of nodes; afterward: nodeid T_weight for each line
		string T_SaveFoldScar = PatchFold + "Patch_T_info.txt";
		// first line number of nodes; afterward: nodeid T_weight for each line
		string T_SaveFoldNorm = PatchFold + "Patch_T_info_norm.txt";
		// first line number of edges; afterward: \t nodeid first_node_id cellid_of_2nd_node  distance_in_mm for one line, 
		//                              and then: nodeid first_node_id cellid_of_2nd_node nlink_weight for another line 
		string N_SaveFold = PatchFold + "Patch_N_info.txt";

		//--------------------load image-----------------------------------------------------------
		zxhImageData SourceImage, LABloodPoolGaussianBlur, MeshWallLabel, T_PatchImage, N1_PatchImage, N2_PatchImage;
		...
		T_PatchImage.NewImage(3, T_newsize, spacing111, SourceImage.GetImageInfo());
		N1_PatchImage.NewImage(3, N_newsize, spacing111, SourceImage.GetImageInfo());
		N2_PatchImage.NewImage(3, N_newsize, spacing111, SourceImage.GetImageInfo());

		zxhImageData *ImageArray_T[] = { &SourceImage, &LABloodPoolGaussianBlur, &T_PatchImage };
		zxhImageData *ImageArray_N1[] = { &SourceImage, &LABloodPoolGaussianBlur, &N1_PatchImage };
		zxhImageData *ImageArray_N2[] = { &SourceImage, &LABloodPoolGaussianBlur, &N2_PatchImage };
		//-------------------------------------------------------------------------------
		...
		for (int ptId = 0; ptId < iNumOfMeshPoints; ptId++)
		{
			...
			if (PatchExtractByWorldCoordinate::GeneratePatchExtractByWorldWithRandOffset_test(InputWorldCoord, PatchInfo, 0, ImageArray_T, nNodeSaved) == false)
				continue;
			nNodeSaved++;
		...
		string T_ResultName = PatchFold + "Patch_T.nii.gz";
		string N1_ResultName = PatchFold + "Patch_N1.nii.gz";
		string N2_ResultName = PatchFold + "Patch_N2.nii.gz";
		zxh::SaveImage(&T_PatchImage, T_ResultName);
		zxh::SaveImage(&N1_PatchImage, N1_ResultName);
		zxh::SaveImage(&N2_PatchImage, N2_ResultName);
