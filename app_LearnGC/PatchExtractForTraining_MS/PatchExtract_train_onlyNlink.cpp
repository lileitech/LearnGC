...
int main(int argc, char* argv[])
{
	...
	zxhImageData* ImageArray_N1[5] = { &SourceImage, &LABloodPoolGaussianBlur, &N_PatchImage1, &N_PatchImage2, &N_PatchImage3 };
	...
	for (int ptId = 0; (ScarNumIdex < Num_Scar) || (NormalNumIdex < Num_Normal); ptId++)
	{
		...
		if (PatchExtractByWorldCoordinate::GeneratePatchExtractByWorldWithRandOffset(InputWorldCoordRand, PatchInfo, randoffset, ImageArray_T1, ScarNumIdex + NormalNumIdex) == false)
			continue;//for t-link
		if (PatchExtractByWorldCoordinate::GeneratePatchExtractByWorldWithRandOffset(N_InputWorldCoordRand, PatchInfo, randoffset, ImageArray_N1, ScarNumIdex + NormalNumIdex) == false)
			continue;;//for n-link
	...
	zxh::SaveImage(&T_PatchImage2, PatchFold + "Patch_N1_scale2.nii.gz");
	zxh::SaveImage(&N_PatchImage2, PatchFold + "Patch_N2_scale2.nii.gz");

	zxh::SaveImage(&T_PatchImage3, PatchFold + "Patch_N1_scale3.nii.gz");
	zxh::SaveImage(&N_PatchImage3, PatchFold + "Patch_N2_scale3.nii.gz");
